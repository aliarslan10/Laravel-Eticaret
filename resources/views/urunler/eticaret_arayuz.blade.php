              ...
              <article role=""> <!-- class : vitrin-stili -->
                ...
                <div class="col-sm-6 col-md-3" style="margin-bottom: 20px;">
	                <div class="vitrin-cerceve">
	                    <div class="col-sm-12 vitrin-gorsel" style="padding: 0; height: 140px;">
	                    	<img style="width: 100%;  height: 140px;" src='@if(($icerik->resim_linki)==null) {{ asset("img/hazirlaniyor.png") }} @else 
	                    	{{ asset("img/urunler/$icerik->resim_linki") }} @endif' alt="{{$icerik->resim_linki}}" title="{{$icerik->resim_linki}}">
	                    </div> 
	                    <div class="col-sm-12 vitrin-baslik" style="font-size: 14px; height: 20px; overflow: hidden;">{{$icerik->sayfa_adi}} </div>
	                    <div class="col-sm-12 yildiz"> <img width="100%" src="{{ asset('img/yildiz.png') }}"> </div>
	                    <div class="vitrin-tanitim" style="height: 70px; overflow: hidden;"> <p> <?=strip_tags($icerik->sayfa_icerik)?> </p> </div>
	                    <div class="clearfix"></div>
	                    <div class="vitrin-satis">
	                        <div class="pull-left" style="color: white; font-size: 12px;"><i class="glyphicon glyphicon-tags"> </i> &nbsp; 
								@if($icerik->indirimli_fiyat) 
									{{$icerik->indirimli_fiyat}} TL
								@else 
									{{$icerik->fiyat}} TL 
								@endif
							</div>
	                        <div class="pull-right">
	                         <a href="javascript:;" productcode="{{$icerik->urun_id}}" productname="{{$icerik->sayfa_adi}}"
	                         style="padding: 3px; font-size: 12px; text-align: center;" class="inputbtn addtocartbutton"> <i class="glyphicon glyphicon-shopping-cart"></i> Sepete Ekle
                        	<meta name="csrf-token" content="{!! Session::token() !!}"> </a> </div>
	                        <div class="clearfix"></div>
	                    </div>
	                </div>
                </div>
