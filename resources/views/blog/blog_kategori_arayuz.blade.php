...
@foreach($blogKategoriler as $menu)
	<li><i style="color: gray;" class="glyphicon glyphicon-chevron-right"></i> <a href="{{$menu->kategori_link}}">{{$menu->menu_adi}}</a></li>
@endforeach
</ul>
@stop

@section('blog_yazi_baslik') <h2>{{$baslik}}</h2> @stop
@if($blogSonIcerikler[0])
@section('blog_yazi')
	...
	<div class="col-sm-4"> <img style="max-height:100px; width:100%;" src='@if(($icerik->resim_linki)==null) img/gorsel-hazirlaniyor.jpg @else {{ asset("img/blog/$icerik->resim_linki") }} @endif' alt="{{$icerik->resim_linki}}" title="{{$icerik->resim_linki}}"></div>
	<div class="col-sm-8">
...
@stop
@else
@section('blog_yazi') <div class="alert alert-danger"><i class="fa fa-info-circle"></i> Bu kategoriye henüz içerik girilmemiş. </div> @stop
@endif